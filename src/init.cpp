...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Lyrabar version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  lyrabard [options]                     " + "\n" +
                  "  lyrabard [options] <command> [params]  " + _("Send command to -server or lyrabard") + "\n" +
                  "  lyrabard [options] help                " + _("List commands") + "\n" +
                  "  lyrabard [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "lyrabar:"))
                fCommandLine = true;
...
int main(int argc, char* argv[])
{
    ...
    // Connect lyrabard signal handlers
    noui_connect();
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: lyrabar.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: lyrabard.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 4046 or testnet: 14046)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 4047 or testnet: 14047)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Lyrabar Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Lyrabar is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Lyrabar version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDaemon)
        fprintf(stdout, "Lyrabar server starting\n");
    ...
    // requested to kill lyrabar-qt during the last operation. If so, exit.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Lyrabar") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Lyrabar to complete") << "\n";
