    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Lyrabar address (e.g. LdNiYrYaCaXt8pVy2ygSeDaXXedz26jr15)"));
#endif
        ...
        if(entry)
        {
            if(entry->validate())
            {
                SendCoinsRecipient sxRecipient = entry->getValue();
                    if(sxRecipient.address.length() > 40){
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Lyrabar address"));
        }
        else
        {
